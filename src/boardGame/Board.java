...
public class Board {
    ...
    public Board(int rows, int columns) {
        if(rows < 1 || columns < 1){
            throw new BoardException("Erro creating Board: There must be at least 1 or more rows or columns");
        }
        this.rows = rows;
    ...
    public void placePiece(Piece piece, Position position){
        if(thereIsAPiece(position)){
            throw new BoardException("This is already a piece on position "+position);
        }
